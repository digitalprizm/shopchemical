...
def all_product():
	...
	item_list = frappe.db.sql("""select i.name,i.item_code,i.item_group,i.website_image,i.image,i.thumbnail,
		i.route as item_route, COALESCE(i.delivery_time, '-') AS delivery_time,ig.route as group_route,i.show_get_quote,i.msds,i.brand, i.stock_uom,
		case 
			WHEN 1 = 1 THEN 
...
def get_all_product():
    ...
    # 	fields = ("name", "description", "website_image"))
	item_list = frappe.db.sql("""select i.name,i.item_code,i.item_group,i.website_image,i.image,i.thumbnail,
		i.route as item_route, COALESCE(i.delivery_time, '-') AS delivery_time,ig.route as group_route,i.show_get_quote,i.msds,i.brand from tabItem i, `tabItem Group` ig where i.show_in_website=1 and i.item_group = ig.name""",as_dict=1)
	return item_list

@frappe.whitelist(allow_guest=True)
def get_brandwise_item(brand):
	item_list = frappe.db.sql("""select name,item_code,item_group,website_image,image,thumbnail,
		route as item_route, COALESCE(i.delivery_time, '-') AS delivery_time,show_get_quote,msds,brand from tabItem where show_in_website=1 and 
...
def get_item_detail(item_code):
	item_list = frappe.db.sql("""select name,item_code,item_group,website_image,image,thumbnail,
		route as item_route, COALESCE(i.delivery_time, '-') AS delivery_time,show_get_quote,msds,brand from tabItem where item_code='{0}'""".format(item_code),as_dict=1)
	return item_list
